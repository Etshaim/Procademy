...
CScreenDib		g_cScreenDib(dfWINDOW_WIDTH, dfWINDOW_HEIGHT, 32);			// 전역으로 ScreenDib 생성
...
void Draw(void)
{
	...
	{
		iMapX = g_pPlayerObject->GetCurX();
		iMapY = g_pPlayerObject->GetCurY();
	}

	// draw 위치 구하기
	g_cMap.SetDrawPos(iMapX, iMapY);
	...
	g_cMap.DrawTile(bypDest, iDestWidth, iDestHeight, iDestPitch);
...
BOOL SpriteLoad(void)
{
	...
	if (!g_cSpriteDib.LoadDibSprite(e_SPRITE::eMAP, L"Sprite_Data\\Tile_01.bmp", 0, 0))							return FALSE;
